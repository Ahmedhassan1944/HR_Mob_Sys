...
/**
 * Permanently deletes an event's row from the sheet and removes it from Google Calendar.
 */
function api_deleteCalendarEvent(eventId) {
  ...
  try {
    ...
    api_writeLog_(rowData[headers.indexOf('CandidateID')], Session.getActiveUser().getEmail(), 'Calendar Event Deleted: ' + rowData[headers.indexOf('Title')]);
    CacheService.getScriptCache().remove('dashboard_data');
