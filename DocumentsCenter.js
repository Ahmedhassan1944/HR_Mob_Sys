 ...
 *
 * Progress weight distribution (designed so no stage stays frozen):
 *   0% →  5%  — Scanning sheets & building document list
 *   5% → 65%  — Downloading files from Drive (per-file, every file)
 *  65% → 68%  — Announcing ZIP build start
 *  68% → 85%  — Utilities.zip + base64 encode (announced before & after)
 *  85% → 93%  — Preparing download link (base64 encode tail + cleanup)
 *  93% → 98%  — Finalizing response
 *  98% → 100% — Complete
 *
...
function api_batchDownloadZip(candidateIdsJson, docTypesJson, batchId) {
  ...
  try {
    ...
    setProgress_(batchId, {
      stage: 'scanning',
      percent: 2,
      message: 'Reading document records…',
      filesFound: 0,
      filesDownloaded: 0
    });
    ...
    // ── Stage 3%: Sheet data loaded ───────────────────────────────────────
    setProgress_(batchId, {
      stage: 'scanning',
      percent: 3,
      message: 'Scanning document records…',
      filesFound: 0,
      filesDownloaded: 0
    });
    ...
    var totalFiles  = Math.max(matchCount, 1); // avoid division by zero
    ...
    // ── Stage 5%: Scan complete, count known ──────────────────────────────
    setProgress_(batchId, {
      stage: 'scanning',
      percent: 5,
    ...
    // ── Stage 5% → 65%: Download each file from Drive ─────────────────────
    ...
    for (var k = 0; k < matchedDocs.length; k++) {
      ...
      // Announce BEFORE fetching so the bar moves at the start of each file
    ...
    // ── Stage 65%: All files downloaded, announce ZIP start ───────────────
    setProgress_(batchId, {
      stage: 'zipping',
      percent: 65,
      message: 'All files downloaded — building ZIP archive…',
      filesFound: matchCount,
    ...
    setProgress_(batchId, {
      stage: 'encoding',
      percent: 78,
      message: 'ZIP archive ready — encoding for download…',
      filesFound: matchCount,
      filesDownloaded: blobs.length
    });
    ...
    // ── Stage 85%: Encoding complete ──────────────────────────────────────
    setProgress_(batchId, {
      stage: 'encoding',
      percent: 85,
      message: 'Encoding complete — preparing download link…',
      filesFound: matchCount,
      filesDownloaded: blobs.length
    });
    ...
    setProgress_(batchId, {
      stage: 'finalizing',
      percent: 93,
      message: 'Finalizing your download…',
      filesFound: matchCount,
      filesDownloaded: blobs.length
    });

    // ── Stage 98%: Response assembled ─────────────────────────────────────
    setProgress_(batchId, {
      stage: 'finalizing',
      percent: 98,
